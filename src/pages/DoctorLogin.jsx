...
import { Stethoscope, Lock, User, ChevronRight } from 'lucide-react';
    ...
    try {
      ...
      if (res.ok) {
        if (data.role === 'doctor') {
          ...
          setError('عذراً، هذا الحساب ليس لديه صلاحيات الطبيب');
        }
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 relative p-6">
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
        <div className="bg-primary p-8 text-center text-white">
          ...
          <p className="text-primary-100 opacity-90 text-sm mt-2">تسجيل الدخول لعرض جدول المواعيد</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="p-8 space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" 
                autoFocus
                className="w-full bg-slate-50 border border-slate-200 rounded-xl pr-10 pl-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
          ...
          <button type="submit" className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-3 rounded-xl transition-all shadow-lg active:scale-95 mt-4">
            تسجيل الدخول
          </button>
        </form>
      </div>
